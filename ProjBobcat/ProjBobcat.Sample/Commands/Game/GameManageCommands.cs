...
public class GameManageCommands(ILogger<GameManageCommands> logger)
{
    ...
    public async Task RepairGameAsync(
        ...
        [Range(0, 2)] [Argument("mirror", Description = "Download mirror index, use [game mirrors] to reveal mirror indexes")] int mirrorIndex = 0,
        [Range(1, 32)] [Argument("parallel", Description = "Parallel tasks")] int parallel = 8,
        [Range(1, 128)] [Argument("download-threads", Description = "Download threads")] int downloadThreads = 32,
        [Range(1, 16)] [Argument("download-parts", Description = "Download parts")] int downloadParts = 8,
        [Option("verbose", Description = "Enable verbose mode")] bool verbose = false,
        [Option("disable-verify", Description = "Disable file verification")] bool disableVerify = false,
        [Option("uncheck-version", Description = "Disable Version Info checker")] bool uncheckVersion = false,
        [Option("uncheck-asset", Description = "Disable Asset Info checker")] bool uncheckAsset = false,
        [Option("uncheck-lib", Description = "Disable Library checker")] bool uncheckLib = false)
    {
        ...
        if (!uncheckAsset)
        {
            ...
            if (vm == null)
            {
                logger.LogCritical("Failed to fetch Version Manifest from remote. Please check your network connection.");
                return;
            }
        ...
        {
            progressVal = args.ProgressPercentage;
        };
