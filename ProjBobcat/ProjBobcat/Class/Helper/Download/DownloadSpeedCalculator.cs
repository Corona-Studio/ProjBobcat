...
public class DownloadSpeedCalculator
{
    ...
    public double AddSample(long bytesReceived)
    {
        ...
        // 添加新样本
        _speedSamples.Enqueue((now, bytesReceived));

        // 移除过期样本 - 这部分仍需要锁定以保证一致性
        // ConcurrentQueue不支持从头部删除元素，我们需要定期清理
        if (_speedSamples.Count > _maxSamples)
        {
            lock (_cleanupLock)
            {
                // 再次检查，因为可能另一个线程已经清理过
    ...
    private void CleanupOldSamples(DateTime now)
    {
        while (_speedSamples.Count > _maxSamples ||
              (_speedSamples.TryPeek(out var oldestSample) &&
               (now - oldestSample.Time) > _windowDuration))
        {
            // 尝试移除队列头部元素
            if (!_speedSamples.TryDequeue(out _))
                break; // 如果无法移除，中断循环
        }
    }
