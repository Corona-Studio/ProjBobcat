...
namespace ProjBobcat.DefaultComponent.Launch
{
    public class DefaultLaunchArgumentParser : LaunchArgumentParserBase, IArgumentParser
    {
        ...
        public string ParseAdditionalArguments()
        {
            ...
            if ((LaunchSettings.GameArguments.Resolution?.Height ?? 0) > 0 && (LaunchSettings.GameArguments.Resolution?.Width ?? 0) > 0)
            {
                sb.Append($"--width {GameProfile.Resolution?.Width ?? LaunchSettings.GameArguments.Resolution.Width}").Append(" ");
                sb.Append($"--height {GameProfile.Resolution?.Height ?? LaunchSettings.GameArguments.Resolution.Height}").Append(" ");
            }
            else
            {
                sb.Append($"--width {GameProfile.Resolution?.Width ?? LaunchSettings.FallBackGameArguments.Resolution.Width}").Append(" ");
                sb.Append($"--height {GameProfile.Resolution?.Height ?? LaunchSettings.FallBackGameArguments.Resolution.Height}").Append(" ");
            }
